...
`함수 네이밍 중요성`
적절한 함수 이름은 함수 내부 코드를 이해하지 않고도 함수의 역할을 파악할 수 있다.
→ 코드의 가독성이 좋아짐.
...
위 예제에서는 foo를 선언한 적도 없고 할당한 적도 없다.
그럼 뭐지? → foo는 자바스크립트 엔진이 암묵적으로 생성한 식별자이다.
...
🤖 GPT
일반 - 많은 콜백 함수와 이벤트 핸들러에 널리 사용됨.
이름을 붙이는 경우 - 재귀 호출이 필요한 경우.

**함수 생성 시점과 함수 호이스팅**
함수 선언문 또한 코드의 선두로 끌어 올려진 것처럼 동작한다.
...
**매개변수의 최대 개수**
매개변수는 최대 3개 이상을 넘지 않는 것이 좋고, 만약 그 이상의 매개변수가 필요하다면 하나의 매개변수를 선언하고 객체를 인수로 전달하는 것이 좋다.
...
1번째 역할 - 함수의 실행을 중단하고 함수 몸체를 빠져나감.
2번째 역할 - 반환문은 return 키워드 뒤에 오는 표현식을 평가해 반환.
...
call by value 값에 의한 호출, 전달


call by reference 참조에 의한 호출, 전달
...
obj
참조 값이 복사되어 매개변수에 전달.
함수 몸체에서 참조 값을 통해 겍체를 변경할 경우 원본이 훼손.
...
**순수 함수와 비순수 함수**
순수 함수 - 부수 효과가 없는 함수(외부 상태 의존X, 변경X)
